...
| **`BEGIN IMMEDIATE` Transactions** | SQLite always uses SERIALIZABLE isolation, but lock timing matters. BEGIN IMMEDIATE acquires RESERVED lock at transaction start, blocking other writers immediately while still allowing concurrent reads. Prevents "check-then-act" bugs where two concurrent requests both read balance=1000, both pass validation, then both try to write—causing lost updates or SQLITE_BUSY_SNAPSHOT errors. | BEGIN DEFERRED allows concurrent transactions to read stale data before acquiring write lock, enabling "check-then-act" race condition. BEGIN EXCLUSIVE would block readers unnecessarily. BEGIN IMMEDIATE serializes writers from the start, eliminating the race window entirely. |
...
| **String Amounts in API** | Preserves decimal precision. Standard practice for financial APIs. | Requires parsing/validation, but avoids float precision errors. 
